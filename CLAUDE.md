...
single-author project. Commit to `master` and push. Do not offer a branch, do
not ask whether one would be preferable, and do not treat "you are on the
default branch" as a reason to hesitate — here it is the only branch, by
choice.

The rule that *does* apply is the one above: commit, and commit promptly.
Ceremony around how the commit lands is the opposite of the problem this
...
that doc is wrong and should be fixed rather than followed. This is not a
preference to be re-litigated per session: a tool that defaults to creating one
gets the flag that stops it, not a venv.
