...
published: false
description: A three-cloud agent mesh with no stored secrets, and a measurement most agent-identity writing only asserts — permissions narrowing across a federation boundary, in both directions, with AWS naming which layer refused. Plus why IAM outbound identity federation closed the last keyless gap in the matrix.
tags: aws, bedrock, a2a, aiagents
---

This article looks at what AWS gets right in cross-cloud agent auth, measured on a deployed three-cloud A2A mesh with no long-lived credential anywhere in it.
...
Discovery is a separate action from invocation. A policy granting only `InvokeAgentRuntime` denies the agent-card fetch however the resources are written, which surfaces nowhere near auth.
...
#### AgentCore Is the Only Ingress That Takes a Foreign Token

Here is the structural point, and AWS comes out of it well.

Three clouds means six directed cross-cloud edges, each settled independently by what the caller can present and what the callee will accept. Four clouds is twelve. The integration count grows as N², and every cell is somebody's decision to support somebody else.
...
Cloud Run's invoker check validates Google-signed ID tokens and nothing else. Container Apps ingress takes Entra.

**If every cloud's ingress did what AgentCore's does, the matrix would collapse from N² to N.** Each cloud publishes one issuer for its workloads — all three already do. Each cloud accepts any issuer, subject to conditions you write. Six edges become three mints and three accept-configurations, and a fourth cloud costs one of each instead of six new integrations.

That is a change to two products, not a change to the industry, and AWS has already made it on both sides.

One scoping note. AWS's integration guide for Microsoft Entra states it supports Entra v1.0 and v2.0 access and ID tokens "that do not have any custom claims." That constraint is documented on the Entra-specific page; the general authorizer documentation describes discovery URL, audiences and clients without a corresponding restriction. Test before relying on custom claims through any issuer.
...
So the comment in the repository describes a date rather than a property of the two clouds. That is worth saying out loud because it is the failure mode this project has documented before: a boundary measured honestly, written down clearly, and read a year later as though it were permanent.
...
Not on this shape of deployment.

SPIRE's trust model starts with a node it can attest, and AgentCore, Container Apps and Cloud Run do not give you one. There is an RFC for serverless support, [spiffe/spire#1843](https://github.com/spiffe/spire/issues/1843), opened September 2020 and still a proposal. What it proposes to attest *with* is the interesting part — for AWS Lambda, a SigV4-signed `GetCallerIdentity` query the server replays. That is a primitive this framework already calls directly.

Adopting SPIRE would not remove a single federation. AWS still needs a trust policy, Entra still needs a federated credential, Cloud Run still needs an invoker binding. SPIRE changes which issuer those three trust; it does not reduce their number. And it puts a JWKS endpoint you host in front of all three legs, so a mesh whose legs currently fail independently gains one shared dependency.
...
Worth noting because it is the kind of detail that gets lost. AgentCore Identity's outbound Microsoft credential provider is configured with a stored `clientSecret`:
...
That is a different mechanism from outbound identity federation — an OAuth credential provider for access to Microsoft resources, not workload identity — so the two are not in conflict. But AWS ships a keyless way for a workload to authenticate to Entra, and its agent identity service configures that leg with a client secret anyway. This mesh does it with a federated credential and none.
...
AWS is the only one of the three whose agent ingress will validate a token from an issuer it does not own, and since November 2025 it is also an OIDC issuer for its own workloads. It has done both halves of the change that would make cross-cloud agent auth linear instead of quadratic.

And the one claim in this whole discourse that a deployment can prove rather than assert is an AWS feature: permissions narrow across a federation boundary, in both directions, with the provider naming which layer refused.
