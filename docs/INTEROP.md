...
> **Domain change, 2026-08-12.** This repo carried a currency-conversion mesh
> until this date and now carries three research agents judged against a
> rubric. Every finding below is dated and was measured on the currency mesh
> unless it says otherwise. They are protocol findings, so they survive the
> domain change — but the *numbers* were taken against a different payload, and
...
`to_a2a()` attaches the agent's reply as a task artifact **and** leaves it in
task history. `clients/a2a_sdk.py` reads every carrier the spec allows, which
it must: Agent Framework's `A2AExecutor` puts the reply *only* in history with
artifacts empty, so a client reading artifacts alone gets an empty string from
Microsoft. Read both, and Google's reply arrives twice.

**Why it was invisible for months.** The currency parser indexed quotes by
target currency, so the duplicate JSON object overwrote its twin and the answer
was correct. Nothing in nine green matrix cells could have shown it. Under a
research draft the body doubles: GCP returned 202 words of text the other two
clouds returned in 98, `concision` scored a compliant draft as a 100% overrun,
and the judge ranked on an artefact of the transport.

Fixed by deduplicating in `_task_texts` — one reply carried in two envelopes is
one reply — plus a live test asserting all three serving stacks return the same
canned text at the same length. That comparison is the cheapest detector for
the whole class, and it only works because `direct` mode makes all three
byte-identical.

Not filed upstream. It is arguably conformant on ADK's side; the defect is in
the combination, which is the kind of thing only an off-diagonal test finds.
...
> **Superseded 2026-08-12: the MCP path is gone.** The stdio server,
> `coordinator/mcp_stdio.py` and the Frankfurter provider were removed as
> scaffolding carried over from the two-cloud predecessors, and GCP's `llm`
> mode now registers `get_exchange_rate` as a native ADK function tool —
> the same signature the Strands and Agent Framework agents register, so the
> third defect above cannot recur in the form it took.
>
> The findings above stand as measured on 2026-08-09; they are why the
> replacement keeps the tool contract identical across all three clouds. Two
> things followed from the removal: the `mcp<2` pin left the root Dockerfile
> with the dependency, and **the GCP `llm` leg has not been re-deployed since
> the change** — the hosted `llm` results recorded here were produced by the
> MCP path, and nothing has re-verified the function-tool path beyond a local
> construction test.
...
Measured. This was the reason `mcp<2` was pinned in the root `Dockerfile`; the
pin is gone as of 2026-08-12, because the dependency is. Removing the MCP
scaffolding removed the only import path to `google.adk.tools.mcp_tool`, so the
image no longer stands on this defect. **The defect itself is untested against
anything newer** — it was not retested and then declared fixed, it was routed
around, which is the weaker of the two outcomes and worth naming as such.
